            ...
            if st.button("开始检测", type="primary"):
                with st.spinner("检测中..."):
                    ...
                    if model:
                        ...
                        if len(boxes) > 0:
                            st.success(f"检测到 {len(boxes)} 个空货架")
                            # 创建简单的结果 DataFrame
                            data = []
                            for box in boxes:
                                cls = int(box.cls[0])
                                ...
                                data.append({"检测类别": cn_name, "置信度": f"{conf:.2f}"})
    ...
    if 'results_data' in st.session_state and st.session_state['results_data']:
        ...
        if total_class_counts:
            ...
            class_stats_df = pd.DataFrame([
                {"检测类别": cls, "检测数量": cnt} for cls, cnt in sorted(total_class_counts.items(), key=lambda x: x[1], reverse=True)
            ])
...
with tab4:
    ...
    本项目是基于 **YOLO (You Only Look Once)** 系列模型的 **超市空货架检测系统**，用于检测超市货架图像中的 **空货架** 等目标。

    ### ✨ 主要功能
    - **多模型兼容**: 无缝支持 YOLOv8, YOLO11, 以及最新的 YOLO26 模型。
    - **高精度实时检测**: 针对超市空货架数据集进行了训练与优化，支持图片和视频的快速检测。
