...
```bash
# 从上次中断的地方继续训练
python train.py --model runs/train/Supermarket_Empty_Shelf_Detector/yolo11s_supermarket_empty_shelf/weights/last.pt --resume
```
...
每个模型的训练结果保存在 `runs/train/{data_name}/{model_name}_supermarket_empty_shelf/` 目录下：
...
- **Q: 训练结果保存在哪里？**
  - A: 默认保存在 `runs/train/` 目录下。每个实验会根据模型名称生成一个子文件夹，例如 `runs/train/Supermarket_Empty_Shelf_Detector/yolo11s_supermarket_empty_shelf`。
- **Q: 如何查看训练过程的可视化图表？**
...
| `--data`   | `../data/Supermarket_Empty_Shelf_Detector/data.yaml` | 数据集配置文件路径             |
| `--epochs` | `100`                         | 训练总轮数                 |
...
本项目使用 Roboflow 开源超市空货架数据集，包含 1 类检测目标：
...
```
data/Supermarket_Empty_Shelf_Detector/
├── train/images/      # 训练集图片
├── train/labels/      # 训练集标签
├── valid/images/      # 验证集图片
├── valid/labels/      # 验证集标签
