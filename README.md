...
> **🔗 开源地址**：
>
> - **CNB**：[点击访问](https://cnb.cool/DY-DaoYi/YOLO_Supermarket_Empty_Shelf_Detector)
> - **GitHub**：[点击访问](https://github.com/DY-DaoYi/YOLO_Supermarket_Empty_Shelf_Detector)
>
...
> 3. **（赠品）数据集**：本项目使用 [Roboflow Supermarket Empty Shelf Detector Dataset](https://universe.roboflow.com/fyp-ormnr/supermarket-empty-shelf-detector/dataset/4) 的开源数据集。
...
- **🇨🇳 全中文支持**: 从界面菜单到检测框标签（如"空货架"等），全部采用中文显示，符合国内用户习惯。
...
为了让系统能够识别超市货架图像中的目标，你需要加载训练好的 YOLO 模型文件（`.pt`）。
...
2. **放置模型**：将你训练得到的 `best.pt` 文件复制到 `models/` 文件夹中（建议重命名为清晰的名称，如 `yolo11m_supermarket_empty_shelf.pt`）。
...
本项目使用 [Roboflow Supermarket Empty Shelf Detector Dataset](https://universe.roboflow.com/fyp-ormnr/supermarket-empty-shelf-detector/dataset/4) 开源数据集，主要用于检测 **超市空货架**。
...
2. **训练规模**:
  ...
  - **训练轮数**: 100 Epochs / 模型
3. **实际耗时与成本**:
  - *总耗时*: 02:12:56 (约 2.22 小时)
  - *云服务器费率*: ¥9.35 / 小时
  - **总算力成本**: **2.22 小时 x ¥9.35 ≈ ¥20.72**
